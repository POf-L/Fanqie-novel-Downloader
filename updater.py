...
class UpdateChecker:
    ...
    def _compare_timestamp_versions(self, latest: str, current: str) -> bool:
        ...
        try:
            ...
            # 若当前版本不是时间戳格式，而最新是时间戳或更高，则认为有更新
            if not self._is_timestamp_version(current):
                return True

            # 若最新不是时间戳而当前是，则不更新
            if not self._is_timestamp_version(latest):
                return False

            # 直接比较时间戳字符串（YYYY.MM.DD.HHMM），字典序可用
            return latest_ts > current_ts
        except Exception as e:
            print(f"版本比较异常: {e}")
            return False
...
class AutoUpdater:
    ...
    def _get_platform_asset(self, assets: list, prefer_debug: Optional[bool] = None) -> Optional[Dict[str, Any]]:
        """
        根据平台与变体选择合适的下载文件

        Args:
            assets: 资源列表
            prefer_debug: 是否优先选择 Debug 变体（为 None 时根据渠道推断）
        ...
        # 若未指定 prefer_debug，则按渠道推断（beta/dev -> debug）
        if prefer_debug is None:
            prefer_debug = (self.preferred_channel in ('beta', 'dev'))
        ...
        if not running_packaged:
            for asset in assets:
                if asset.get('is_source'):
                    return asset
            # 退而求其次：选择不含明显平台关键字的通用zip（优先/排除 -debug）
            candidate_generic = []
            for asset in assets:
                ...
                if name_l.endswith('.zip') and not any(k in name_l for k in (platform_keywords['win32'] + platform_keywords['darwin'] + platform_keywords['linux'])):
                    candidate_generic.append(asset)
            if candidate_generic:
                # 按 debug 偏好选择
                for a in candidate_generic:
                    n = a['name'].lower()
                    if prefer_debug and '-debug' in n:
                        return a
                    if not prefer_debug and '-debug' not in n:
                        return a
                return candidate_generic[0]

        # Windows 平台按名称精确选择标准版/调试版 EXE
        if platform == 'win32':
            stable_exe = 'TomatoNovelDownloader.exe'
            debug_exe = 'TomatoNovelDownloader-debug.exe'
            # 允许外部覆盖偏好；未指定时依据渠道推断
            if prefer_debug is None:
                prefer_debug = (self.preferred_channel in ('beta', 'dev'))
            target_order = [debug_exe, stable_exe] if prefer_debug else [stable_exe, debug_exe]
            ...
            # 兜底：任意 .exe，按是否包含 -debug 优先
            best = None
            for asset in assets:
                name_l = asset.get('name', '').lower()
                if name_l.endswith('.exe'):
                    if prefer_debug and '-debug' in name_l:
                        return asset
                    if not prefer_debug and '-debug' not in name_l:
                        return asset
                    if best is None:
                        best = asset
            if best:
        ...
        # 兜底：任意zip（按 debug 偏好）
        zip_candidates = [a for a in assets if a['name'].lower().endswith('.zip')]
        if zip_candidates:
            for a in zip_candidates:
                n = a['name'].lower()
                if prefer_debug and '-debug' in n:
                    return a
                if not prefer_debug and '-debug' not in n:
    ...
    def download_update(self, update_info: Dict[str, Any],
                       ...
                       prefer_debug: Optional[bool] = None,
                       selected_asset_name: Optional[str] = None) -> Optional[str]:
        """
        下载更新
        ...
        Args:
            ...
            prefer_debug: 是否优先选择 Debug 变体
            selected_asset_name: 指定下载的资产文件名（优先使用）
        ...
        try:
            ...
            if selected_asset_name:
                for a in update_info.get('assets', []):
                    if a.get('name', '').lower() == selected_asset_name.lower():
                        asset = a
                        break
            if asset is None:
                asset = self._get_platform_asset(update_info['assets'], prefer_debug=prefer_debug)
            if not asset:
